...
def window_to_image(z_where, window_size, image_size, windows):
    ...
    grid = F.affine_grid(
        theta, torch.Size((n, 1, image_size, image_size)), align_corners=True
    )
    out = F.grid_sample(
        windows.view(n, 1, window_size, window_size), grid, align_corners=True
    )
...
def image_to_window(z_where, window_size, image_size, images):
    ...
    grid = F.affine_grid(
        theta_inv, torch.Size((n, 1, window_size, window_size)), align_corners=True
    )
    out = F.grid_sample(
        images.view(n, 1, image_size, image_size), grid, align_corners=True
    )
    return out.view(n, -1)
