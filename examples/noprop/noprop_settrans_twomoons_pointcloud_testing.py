...
def evaluate_acc(
    model, x, y, num_classes, compute_loss, device, num_samples=1, η: float = 1.0
):
    ...
    for _ in range(num_samples):
        elbo -= compute_loss(model, x, y_, device, η).detach()
    elbo = elbo / num_samples
    probs = F.softmax(elbo.reshape(num_classes, y.size(0)), dim=0)
    pred = probs.argmax(dim=0)
    num_correct = (pred == y).sum().item()

    prob_true, prob_pred = calibration_curve(
        y.cpu().numpy(), probs[1].cpu().numpy().squeeze(), n_bins=5
    )
    ece = np.abs(prob_true - prob_pred).mean()

    return num_correct, ece
...
def train_and_eval(time_emb_dim, embed_dim, batch_size, epoches, optim, flow):
    print("start")
    ...
    print(f"--- TwoMoons using {device} ---")

    samples, labels = two_moons(3 * dataset_size * num_samples)
    print(samples.shape, labels.shape)
    s1 = samples[labels == 0][: dataset_size * num_samples // 2].reshape(
        -1, num_samples, 2
    )
    s2 = samples[labels == 1][: dataset_size * num_samples // 2].reshape(
        -1, num_samples, 2
    )
    ds_train = {
        "image": torch.cat([s1, s2], dim=0),
        "label": torch.cat(
            [torch.zeros(dataset_size // 2), torch.ones(dataset_size // 2)]
        ).long(),
    }

    s1 = samples[labels == 0][
        dataset_size * num_samples // 2 : dataset_size * num_samples
    ].reshape(-1, num_samples, 2)
    s2 = samples[labels == 1][
        dataset_size * num_samples // 2 : dataset_size * num_samples
    ].reshape(-1, num_samples, 2)
    ds_test = {
    ...
    tr_loader = DataLoader(
        TensorDataset(ds_train["image"], ds_train["label"]),
    ...
    te_loader = DataLoader(
        TensorDataset(ds_test["image"], ds_train["label"]),
    ...
    if optim == "ivon":
        ...
        optimizer = torch_optimizer.Lamb(model.parameters(), lr=1e-2, weight_decay=1e-4)
        scheduler = None
    ...
    for ep in range(1, epoches + 1):
            ...
            if optim == "ivon":
                for _ in range(train_samples):
                    with optimizer.sampled_params(train=True):
                        optimizer.zero_grad()
                        total_loss += train_step(model, x, y, device, η) / train_samples
                optimizer.step()
            else:
                optimizer.zero_grad()
                total_loss += train_step(model, x, y, device, η)
                optimizer.step()
        ...
        if ep % 10 == 0:
            ...
            for x, y in te_loader:
                ...
                tot += y.size(0)
                _acc, _ece = evaluate_acc(
                    model, x, y, num_classes, compute_loss, device, num_samples=10, η=η
                )
                acc += _acc
                ece += _ece
            plt.figure(figsize=(4.8, 4.8))
            plt.hist2d(
                *preds.cpu().numpy().reshape(-1, 2).T, bins=64, range=((-2, 2), (-2, 2))
            )
            plt.savefig(f"twomoons_gen_{ep}.png")

            print(
                f" | ACC {100 * acc/tot:4.2f}% | ECE {ece / len(te_loader): 4.2f} | time {time.time()-eval_t0:3.1f}s",
                end="",
            )
        print()
